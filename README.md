...
sdPBRなどのシェーダを作るにあたって新しいレンダリング手法を試す時には、お手本が無いと結果が正しいのかどうかわかりません。そのお手本を作るためのモノです。現状では動画を作って遊ぶための物にはなり得ません。なので、DanceではなくてDayoです。  
...
- `preview shader` 材質の違いをほとんど考慮しない軽いシェーダです
...
`config.json`をいじると材質名に含まれるキーワードと材質の対応付けを変えられますが、エラーチェックなどをやってないので正しく読み込めるjsonファイルでないとどんなエラーが出るか分かりませんから、バックアップを取ってからいじって下さい  
...
- "autoNormal":数値  
  テクスチャの明度勾配に応じた法線マップを自動生成します数値が大きいほど凹凸感が出ます
- "category":"metal" or "subsurface" or "glass"  
  マテリアルのカテゴリを金属・表面下散乱・ガラスから選択します。指定が無い場合は標準マテリアルになります。
- "emission":[数値r,数値g,数値b]  
  マテリアルの発光要素をr,g,bの順に配列で指定します
- "cat":[数値r,数値g,数値b,数値scale]  
  カテゴリ固有パラメータの配列です。現在は表面下散乱のみ対応しており、以下のような意味になります
...
- "lightFalloff":数値  
  発光要素はスポットライト状に広がりを持ちます。広がりの範囲をラジアン単位で指定します。省略された場合、90°になります。あまり狭い場合は双方向パストレーサでないとうまく扱えません
- "lightHotspot":数値  
  発光要素が減衰せずに広がる範囲です。ラジアン単位で指定します。省略された場合、90°扱いになります
- "roughness":[数値T,数値B]
  材質の粗さです。2つの数値はそれぞれ接線,従法線方向に対応し、0～1で指定します
- "IOR":[数値A,数値B]
  材質の屈折率です。実際の屈折率はA+(B/λ[nm])^2としてコーシーの分散公式によって計算されます。材質の屈折率の波長依存性を加味するためです。  
...
- DirectX Tex(desktop_win10を選択)
- DirectX12Tk収録のSimpleMath.cpp,SimpleMath.h,d3dx12.h
- BulletPhysics
- Intel open image denoiser(OIDN)

DirectX Tex, OIDNについては普通にcloneしてビルドしてインクルードパス、ライブラリパスを通すだけです  
