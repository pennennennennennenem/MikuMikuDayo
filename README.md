...
- `relaxLinear`, `relaxAngle` テスト中です。キーフレームに書き込みなどはしないんですが、どういう効果があるのかは数値を適当に変更してからモデルをゆっさゆっさ動かしてみると分かるかもしれません。BulletPhysicsのバージョンが本家MMDと異なる都合上、物理演算の挙動は微妙に違うので、その補填のために実装されています。
...
- 現状では異なるdayoファイル間で長いボーン名・モーフ名を持ったモデルに対するモーションの使いまわしが出来ないので、vmdayo形式を設計する
...
Cereal, ImGuiのプロジェクトへの追加方法は各ドキュメントにある通り、プロジェクトのソースファイルに追加するだけです。ImGuiのbackendsにはimgui_impl_dx12とimgui_impl_win32を使ってください
