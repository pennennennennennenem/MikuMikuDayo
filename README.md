...
- "cat":[数値r,数値g,数値b,数値scale]  
  カテゴリ固有パラメータの配列です。現在は`subsurface`のみ対応しており、以下のような意味になります  
...
- "lightFalloff":数値  
  発光要素はスポットライト状に広がりを持ちます。広がりの範囲をラジアン単位で指定します。  
  省略された場合、π/2になります。あまり狭い場合は双方向パストレーサでないとうまく扱えません
- "lightHotspot":数値  
  発光要素が減衰せずに広がる範囲です。ラジアン単位で0～π/2の範囲で指定します。  
  省略された場合、π/2扱いになります
- "roughness":[数値T,数値B]
  材質の表面の粗さです。2つの数値はそれぞれ接線,従法線方向に対応し、0～1で指定します
- "IOR":[数値A,数値B]
  材質の屈折率です。実際の屈折率はA+(B/λ[nm])^2としてコーシーの分散公式によって計算されます。  
  材質の屈折率の波長依存性を加味するためです。  
