...
VMDファイルに格納されているボーン名を示す文字列はANSIでエンコードされているため、国ごとにVMDファイル内に格納されている文字コードと実際の文字との対応は異なる事になります。これに対応できるよう、VMDファイルが作成された際に使われていたコードページが何であるかを`config.json内`の`vmd_locale`で指定できるようになっています  

デフォルトでは日本語(Shift-JIS)を示すコードページ932が指定されていますが、もし他国語版モデル専用のモーションが正しく扱えない場合は、`config.json`内の`vmd_locale`を変更してみてください
