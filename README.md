...
- Windows11
- GeForce RTX2060以降
  - DirectX Raytracing1.0のハードウェアサポート
  - pixel shaderでのSV_Barycentricsセマンティクスのサポート
- 現状ではRadeonシリーズ+DXR環境でのテストはできてません
...
`spline` ツリーの下に、Catmull-Romスプライン補間により曲線的に補間する軸を設定するためのチェックボックスがあります。チェックボックスがオフの軸は線形補間により補間されます。

- `register` 操作結果をキーフレームに登録します(本家MMDとは違い、registerを押さないと編集フレームを切り替える際に操作内容は破棄されますのでご注意ください)
- `copy` 編集中の補間曲線・補間法を補間曲線用コピーバッファにコピーします
- `paste` 補間曲線・補間法をペーストします
- `init` 補間曲線を直線に初期化します
...
VisualStudio2022でのビルドを念頭にしていますが、VisualStudio2026(18)でビルドする場合は、プロジェクトのプロパティで`全般`-`プラットフォームツールセット`をデフォルトのv145から`Visual Studio 2022(v143)`に変更する必要があります。リストに存在しない場合はVisual Studio Installerで追加してください。
